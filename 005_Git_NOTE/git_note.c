...
git merge 与 git pull的区别

怎么合并到远程分支上, 开发过程中怎么与远程分支同步

命令5: git pull
           作用: 取回远程主机某个分支的更新，再与本地的指定分支合并
           git pull是git fetch后跟git merge;
           语法: git pull  <远程主机名> <远程分支名>:<本地分支名>
           例子:git pull origin next:master //取回origin主机的next分支，与本地的master分支合并
           相当于: 1. git fetch origin;
                   2. git merge origin/next
            
知识点: git fetch 与 git pull的区别, 一个合并, 一个不合并
...
3. git merge: 用于将两个或两个以上的开发历史加入(合并)一起
            用法: git merge dev //合并当前到"当前分支"
